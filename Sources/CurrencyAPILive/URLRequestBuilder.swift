...
struct URLRequestBuilder {
    ...
    func build(_ source: Source) throws -> URLRequest {
        guard var urlComponents = URLComponents(string: self.baseUrl) else {
            throw URLError(.badURL)
        }

        urlComponents.path = source.path
        urlComponents.queryItems = source.queryItems
        ...
        urlRequest.httpMethod = source.method.rawValue
        urlRequest.allHTTPHeaderFields = source.headers
